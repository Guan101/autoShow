<template>
  <div class="boxs">
    <van-nav-bar title="法拉利" left-text="返回" @click="$router.go(-1)" left-arrow> </van-nav-bar>
...
const setCamera = () => {
  ...
  camera = new PerspectiveCamera(45, innerWidth / innerHeight, 1, 3000);
...
const init = async () => {
  ...
  setLight();
  setControls();
  const gltf = await loadFile('/modelss/法拉利/scene.gltf');
