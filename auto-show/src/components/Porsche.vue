<template>
  <div class="boxs">
    <van-nav-bar title="保时捷" left-text="返回" @click="$router.go(-1)" left-arrow> </van-nav-bar>
...
//创建灯光
const setLight = () => {
  directionalLight = new DirectionalLight(0xffffff, 10);
  directionalLight.position.set(-10, 8, 4);
  dhelper = new DirectionalLightHelper(directionalLight, 100, 0xff0000);
  hemisphereLight = new HemisphereLight(0xffffff, 0xffffff, 1);
  hemisphereLight.position.set(0, 20, 0);
  hHelper = new HemisphereLightHelper(hemisphereLight, 1);
...
const setCamera = () => {
  ...
  camera = new PerspectiveCamera(100, innerWidth / innerHeight, 1, 1000);
...
const init = async () => {
  ...
  setLight();
  setControls();
  const gltf = await loadFile('/modelss/保时捷/scene.gltf');
