<template>
  <div class="boxs">
    <van-nav-bar title="宾利" left-text="返回" @click="$router.go(-1)" left-arrow> </van-nav-bar>
...
const setCamera = () => {
  ...
  camera = new PerspectiveCamera(100, innerWidth / innerHeight, 1, 10000);
...
const init = async () => {
  ...
  setLight();
  setControls();
  const gltf = await loadFile('/modelss/宾利/scene.gltf');
