<template>
  <div class="boxs">
    <van-nav-bar title="特斯拉" left-text="返回" @click="$router.go(-1)" left-arrow> </van-nav-bar>
    ...
    <div class="mask">
      ...
      <van-button @click="enterInto" type="danger" size="small">进入车中</van-button>
      <div class="flex">
        <div @click="setCarColor(index)" v-for="(item, index) in colorAry" :key="index" :style="{ backgroundColor: item }"></div>
      </div>
    </div>
  </div>
</template>
...
const setCamera = () => {
  ...
  // 2000是相机最小比
  // camera = new PerspectiveCamera(60, innerWidth / innerHeight, 1, 2000)
...
const init = async () => {
  ...
  setLight();
  setControls();
  const gltf = await loadFile('/modelss/tesla/tesla.gltf');
