<template>
  <div class="boxs">
    <van-nav-bar title="雷克萨斯" left-text="返回" @click="$router.go(-1)" left-arrow> </van-nav-bar>
...
const setLight = () => {
  ...
  dhelper = new DirectionalLightHelper(directionalLight, 20, 0xff0000);
...
const setCamera = () => {
  ...
  camera = new PerspectiveCamera(100, innerWidth / innerHeight, 1, 3000);
...
const init = async () => {
  ...
  setLight();
  setControls();
  const gltf = await loadFile('/modelss/雷克萨斯/scene.gltf');
